...
func main() {
	...
	Shutdown(ctx, wg, session.Session, stop)
}

// Shutdown cancels the context and logs a message.
// TODO: this needs to be handled with an os signal
func Shutdown(ctx context.Context, wg *sync.WaitGroup, irc *twitcirc.IRC  session discord.Client, stop chan os.Signal) {
	<-stop
	ctx.Done()
